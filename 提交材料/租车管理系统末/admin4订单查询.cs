...
namespace 租车管理系统
{
    public partial class admin4订单查询 : Form
    {
        ...
        public void Table()
        {
            ...
            // 用户查询直接在查询界面显示用户的ID以及姓名
            if (Data.flag)
            {
                textBox1.Hide();
                textBox2.Hide();
                label8.Text = Data.UID;
                Dao dao_name = new Dao();
                string sql_name;
                try
                {
                    sql_name = $"select t_user.uname from t_user where t_user.uid = '{Data.UID}';";
                    IDataReader dc_name = dao_name.read(sql_name);
                    if (dc_name.Read())
                    {
                        label9.Text = dc_name[0].ToString();
                    }
                    dc_name.Close();
                    dao_name.DaoClose();
                }
            ...
            {
                // 用户查询
                sql = $"select uor.uid,t_user.uname,uor.rentdate,uor.returndate,st1.storename,st2.storename,t_car_info.cartype,uor.payment from t_user_order uor,t_user,t_store_info st1,t_store_info st2,t_car_info where t_user.uid=uor.uid and st1.storeid=uor.rentstore and st2.storeid=uor.returnstore and t_car_info.carid=uor.carid and t_user.uid='{Data.UID}'";
            }
            else
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Data.flag)
            {
                ...
                if (textBox2.Text != "")
                {
                    sql = sql + " and t_user.uname like '%" + textBox2.Text + "%'";
                }
            }
            ...
            try
            {
                ...
                {
                    flag= false;
                    dataGridView1.Rows.Add(dc[0].ToString(), dc[1].ToString(), get_date(dc[2].ToString()), get_date(dc[3].ToString()), dc[4].ToString(), dc[5].ToString(), dc[6].ToString(), dc[7].ToString());
                }
                if(flag)
                {
