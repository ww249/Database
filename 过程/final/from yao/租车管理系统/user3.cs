...
namespace 租车管理系统
{
    public partial class user3 : Form
    {
        ...
        {
            Dao dao = new Dao();
            string sql = $"exec xu_yue @id = '{textBox1.Text}',@t_date = '{Convert.ToDateTime(dateTimePicker2.Text)}';";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            int day_gap;
            Dao da = new Dao();
            string sql = $"select res.uid,rent.rentdate,res.returndate,car.cartype,type.rentprice from t_car_res res,t_car_rent rent,t_car_info car,t_type_info type where car.carid = rent.carid and res.uid = rent.uid and car.cartype = type.cartype and rent.uid = '{textBox1.Text}';";
            IDataReader dc = da.read(sql);
            if (dc.Read())
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string sql = $"exec return_car @id = '{textBox1.Text}',@r_date = '{DateTime.Now}',@payment = {payment},@over = {over};";
            int n = dao.Execute(sql);
            if (n > 0)
            {
