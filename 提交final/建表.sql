...
index index_car_res               --索引名称
on t_car_res(uid)                 --数据表名称（建立索引的列名）
with 
(
...
CREATE TABLE t_car_rent (
 ...
 FOREIGN KEY (uid) REFERENCES t_user(uid) ON
DELETE CASCADE ON
UPDATE CASCADE);

-- 创建唯一非聚集索引
create unique nonclustered        --表示创建唯一非聚集索引
index index_car_rent              --索引名称
on t_car_rent(uid)                --数据表名称（建立索引的列名）
with 
(
    ignore_dup_key=on,            --表示向唯一索引插入重复值会忽略重复值
    statistics_norecompute=off    --表示启用统计信息自动更新功能
...
-- 创建唯一非聚集索引
create unique nonclustered        --表示创建唯一非聚集索引
index index_pla_carid             --索引名称
on t_car_pla(carid)               --数据表名称（建立索引的列名）
with 
(
    ignore_dup_key=on,            --表示向唯一索引插入重复值会忽略重复值
    statistics_norecompute=off    --表示启用统计信息自动更新功能
...
-- 创建索引
CREATE INDEX Index_pla_store
ON t_car_pla (carstoreid) 

INSERT INTO t_admin (adminid, adminpsw) VALUES ('admin', '123');
INSERT INTO t_admin (adminid, adminpsw) VALUES ('admin1', '123');
...
INSERT INTO t_car_res (uid, cartype, rentdate, returndate, rentstore, returnstore, paydepo) VALUES ('U202301004', '特斯拉MODEL3', '2023-5-12', '2023-5-20', 'S00007', 'S00003', '1');
